                ...
                <tbody>
                    @foreach($menuitems as $key => $menuitem)
                        <tr data-entry-id="{{ $menuitem->id }}">
                            ...
                            <td>
                                @can('menuitem_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.menuitems.show', $menuitem->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('menuitem_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.menuitems.edit', $menuitem->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('menuitem_delete')
                                    <form action="{{ route('admin.menuitems.destroy', $menuitem->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
