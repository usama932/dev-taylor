        ...
        <div class="col-md-12">
            @can('slide_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.slides.create') }}">
                            ...
                            <tbody>
                                @foreach($slides as $key => $slide)
                                    <tr data-entry-id="{{ $slide->id }}">
                                        ...
                                        <td>
                                            @can('slide_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.slides.show', $slide->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('slide_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.slides.edit', $slide->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('slide_delete')
                                                <form action="{{ route('frontend.slides.destroy', $slide->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('slide_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.slides.massDestroy') }}",
